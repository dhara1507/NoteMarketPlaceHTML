...
if(isset($_POST['save'])){
            ...
            if($sell_type=='1'){
                 $query="UPDATE sellernotes SET Status=1,Title ='{$title}',Category='{$category}', DisplayPicture='{$up_note}', NoteType='{$type}', NumberofPages='{$page}', Description='{$desc}', UniversityName='{$inst_name}', Country='{$country}', Course='{$course_name}', CourseCode='{$course_code}', Professor='{$prof_name}', IsPaid='{$sell_type}', SellingPrice='{$price}', NotesPreview='{$note_preview}' WHERE ID='{$the_note}'";
                $update=mysqli_query($conn,$query);
                $query="UPDATE sellernotesattachements SET FilePath='{$up_note}' WHERE NoteID='{$the_note}'";
...
if(isset($_POST['yes'])){
         ...
         $query="UPDATE sellernotes SET Status=2,Title ='{$title}',Category='{$category}', DisplayPicture='{$up_note}', NoteType='{$type}', NumberofPages='{$page}', Description='{$desc}', UniversityName='{$inst_name}', Country='{$country}', Course='{$course_name}', CourseCode='{$course_code}', Professor='{$prof_name}', IsPaid=0, SellingPrice='{$price}', NotesPreview='{$note_preview}' WHERE ID='{$the_note}'";
            $result=mysqli_query($conn,$query);
                if(!$result){
                    die("FAILED".mysqli_error($conn));
                }
