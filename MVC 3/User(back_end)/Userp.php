...
-->
    ...
    if($count>0 && isset($_POST['submit'])){
        ...
        $target_dir="../member/$id/";
        mkdir($target_dir);
        $file=basename($_FILES['prof_pic']['name']);
        $new=$target_dir."DP_".time();
        $p=rename($file,$new);
        move_uploaded_file($_FILES['prof_pic']['tmp_name'],$new);
        ...
        $query1="UPDATE userprofile SET SecondaryEmailAddress='{$email}',DOB='{$date}',Gender='{$gender}',CoutryCode='{$code}',PhoneNumber='{$phn}',ProfilePicture='{$new}',AddressLine1='{$add1}',AddressLine2='{$add2}',City='{$city}',State='{$state}',ZipCode='{$zip}',Country='{$country}',University='{$university}',College='{$college}' WHERE UserID='{$id}'";
