    ...
    if(isset($_POST['save']))
    {
            ...
            if($sell_type=='0'){
               $query="INSERT INTO sellernotes(ID,SellerID,Status,ActionedBy,AdminRemarks,PublishedDate,Title,Category,DisplayPicture,NoteType,NumberofPages,Description,UniversityName,Country,Course,CourseCode,Professor,IsPaid,SellingPrice,NotesPreview,CraetedDate,CreatedBy,ModifiedDate,ModifiedBy,IsActive)VALUES('','{$id}','1','{$id}','','{$dt}','{$title}','{$category}','{$note_pic}','{$type}','{$page}','{$desc}','{$inst_name}','{$country}','{$course_name}','{$course_code}','{$prof_name}',
                ...
                $query="INSERT INTO sellernotesattachements(ID,NoteID,FileName,FilePath,CreatedDate,CreatedBy,ModifiedDate,ModifiedBy,IsActive )VALUES('','{$NOTE_id}','{$title}','{$up_note}','','','','','1')";
                $result=mysqli_query($conn,$query);
                if(!$result){
            ...
            $query="INSERT INTO sellernotes(ID,SellerID,Status,ActionedBy,AdminRemarks,PublishedDate,Title,Category,DisplayPicture,NoteType,NumberofPages,Description,UniversityName,Country,Course,CourseCode,Professor,IsPaid,SellingPrice,NotesPreview,CraetedDate,CreatedBy,ModifiedDate,ModifiedBy,IsActive)VALUES('','{$id}','1','{$id}','','{$dt}','{$title}','{$category}','{$note_pic}','{$type}','{$page}','{$desc}','{$inst_name}','{$country}','{$course_name}','{$course_code}','{$prof_name}',
            '{$sell_type}','{$price}','{$note_preview}','','','','','1')";
            $result=mysqli_query($conn,$query);
                if(!$result){
                    die("FAILED".mysqli_error($conn));
                }
                $NOTE_id=mysqli_insert_id($conn);
                $query="INSERT INTO sellernotesattachements(ID,NoteID,FileName,FilePath,CreatedDate,CreatedBy,ModifiedDate,ModifiedBy,IsActive )VALUES('','{$NOTE_id}','{$title}','{$up_note}','','','','','')";
                $result=mysqli_query($conn,$query);
                if(!$result){
                    die("fail".mysqli_error($conn));
                }
...
//    $select="SELECT sellernotes.ID as Id,sellernotesattachements.NoteID FROM sellernotes LEFT JOIN sellernotesattachements ON sellernotes.ID=sellernotesattachements.NoteID";
